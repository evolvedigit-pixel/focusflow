...
import {
  ...
  Menu, X, CalendarDays, BookOpen, LogOut, CheckSquare, Activity, Settings, BarChart2, Star, Sun,
} from "lucide-react"
...
const navItems = [
  ...
  { href:"/daily",       icon:Sun,             label:"Tâches du jour"  },
  { href:"/goals",       icon:Star,            label:"Objectifs"       },
...
export function Sidebar() {
  ...
  const displayName  = profile?.display_name ?? profile?.name ?? profile?.full_name ?? "Utilisateur"
  ...
  return (
    <>
      ...
      <div className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-4 py-3 border-b border-white/[0.08] bg-background/80 backdrop-blur-xl lg:hidden">
        ...
        <button onClick={() => setMobileOpen(!mobileOpen)} className="p-2 rounded-lg hover:bg-white/[0.05]">
                ...
                return (
                  <Link key={item.href} href={item.href} onClick={() => setMobileOpen(false)}
                    className={cn("flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200",
                      isActive ? "bg-white/[0.08] text-white" : "text-muted-foreground hover:bg-white/[0.04] hover:text-white")}>
            ...
            {navItems.map(item => {
              ...
              return (
                <Link key={item.href} href={item.href}
                  className={cn("relative flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200",
          ...
          <div className="flex items-center gap-3 rounded-xl bg-white/[0.03] p-3">
            ...
            <div className="relative flex-shrink-0">
              ...
              <div className="absolute -bottom-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full text-[10px] font-bold text-white"
                style={{ background:"linear-gradient(135deg,#7c3aed,#6366f1)", border:"1.5px solid #09090b" }}>
                {displayLevel}
              </div>
            </div>
