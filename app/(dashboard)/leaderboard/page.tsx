...
function UserAvatar({ user, size=36 }: { user: LeaderUser; size?: number }) {
  const name = user.display_name ?? user.name ?? user.full_name ?? "?"
...
export default function LeaderboardPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data } = await supabase.from("profiles")
        .select("id, name, full_name, display_name, avatar_url, level, xp, streak, total_focus_hours, sessions_completed")
  ...
  function userName(u: LeaderUser) {
    return u.display_name ?? u.name ?? u.full_name ?? "Anonyme"
  }
              ...
              <div className="mt-4 rounded-xl px-3 py-2.5"
                ...
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <span className="text-xs font-semibold" style={{ color:"#a855f7" }}>
                    {tab==="xp" ? `${(sorted[myRank-2].xp-(me?.xp??0)).toLocaleString()} XP manquants`
                     : tab==="heures" ? `${Math.round(sorted[myRank-2].total_focus_hours-(me?.total_focus_hours??0))}h manquantes`
          ...
          {[sorted[1], sorted[0], sorted[2]].map((user, i) => {
            ...
            return (
              <motion.div key={user.id}
                ...
                className={cn("rounded-2xl p-4 text-center relative overflow-hidden", isMe && "ring-2 ring-violet-500/50")}
                style={{ background:"rgba(255,255,255,0.02)", border:"1px solid rgba(255,255,255,0.07)", marginTop:realRank===1?0:16 }}>
